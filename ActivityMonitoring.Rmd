...
# Overlay the plots using a 1/2 alpha channel so that both are visible
    plot(iplot, col = rgb(0, 1, 0, 1/2), xlab = "Steps", main = "Histogram of Number of Steps Per Day")
    plot(oplot, col = rgb(0, 0, 1, 1/2), add = TRUE)
...
From the graph it appears the mean and median will not change much, however because we imputed based on the weekday it is likely to have changed.
...
The original mean (`r round(amdatamean, 0)`) has increased by over 15%, however the median (`r amdatamedian`) has gone up by less than 6%, which is in line with what we expect for moving low values to around the mean.
...
amdatafull$daytype <- as.factor(c("Weekday", "Weekend")[as.numeric(isweekend) + 1])
